...
**Core Info section:**
- **QR Number** — Type digits only (e.g., `1` or `0042`). Non-digits are stripped automatically. On blur, numbers shorter than 4 digits are zero-padded (e.g., `1` → `0001`). A live availability check runs ~350ms after the last keystroke and shows the result inline (green check if free, red X if taken, spinner while checking). *If the number is taken (solo or within a bulk range), the form shows a red error and blocks submission.*
- **Quantity** — Type how many PCs to create sequentially (1–100). For example, QR Number `10` with quantity `3` creates `0010`, `0011`, `0012`. A preview shows the generated list below the field.
